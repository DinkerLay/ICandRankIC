...
from algs.losses import RankNet_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/pairloss'): os.mkdir('./results/pairloss')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/pairloss/%s' % date):
        save_data = pd.read_csv('./results/pairloss/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/pairloss/%s' % date)
