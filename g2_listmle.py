...
train_window = 12
patience = 20
if not os.path.exists('./results/group_listmle'): os.mkdir('./results/group_listmle')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/group_listmle/%s' % date):
        save_data = pd.read_csv('./results/group_listmle/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/group_listmle/%s' % date)
