...
train_window = 12
patience = 20
if not os.path.exists('./results/listmle'): os.mkdir('./results/listmle')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/listmle/%s' % date):
        save_data = pd.read_csv('./results/listmle/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/listmle/%s' % date)
