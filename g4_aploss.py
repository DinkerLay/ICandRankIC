...
train_window = 12
patience = 20
if not os.path.exists('./results/group_ap'): os.mkdir('./results/group_ap')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/group_ap/%s' % date):
        save_data = pd.read_csv('./results/group_ap/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/group_ap/%s' % date)
