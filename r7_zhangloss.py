...
from algs.losses import Zhang_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/Zhang'): os.mkdir('./results/Zhang')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/Zhang/%s' % date):
        save_data = pd.read_csv('./results/Zhang/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/Zhang/%s' % date)
