...
from algs.losses import ic_loss, rank_reg
...
train_window = 12
patience = 20
if not os.path.exists('./results/rankreg'): os.mkdir('./results/rankreg')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/rankreg/%s' % date):
        save_data = pd.read_csv('./results/rankreg/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/rankreg/%s' % date)
