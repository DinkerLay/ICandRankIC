...
from algs.models import NN2Batchnorm
from algs.losses import RankNet_approx_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/rankapp'): os.mkdir('./results/rankapp')
for ind in range(len(month_list) - train_window - 1):
    ...
    model = NN2Batchnorm(244, drop_out=0.3)
    loss_func = RankNet_approx_loss()
    ...
    if os.path.exists('./results/rankapp/%s' % date):
        save_data = pd.read_csv('./results/rankapp/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/rankapp/%s' % date)
