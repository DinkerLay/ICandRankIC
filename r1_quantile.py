...
from algs.models import NN2net
from algs.losses import quantile_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/quantile'): os.mkdir('./results/quantile')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/quantile/%s' % date):
        save_data = pd.read_csv('./results/quantile/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/quantile/%s' % date)
