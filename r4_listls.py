...
from algs.losses import ListLs_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/listls'): os.mkdir('./results/listls')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/listls/%s' % date):
        save_data = pd.read_csv('./results/listls/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/listls/%s' % date)
