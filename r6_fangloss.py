...
from algs.losses import Fang_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/Fang'): os.mkdir('./results/Fang')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/Fang/%s' % date):
        save_data = pd.read_csv('./results/Fang/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/Fang/%s' % date)
