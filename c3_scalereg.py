...
from algs.models import NN2Batchnorm, NN2net
from algs.losses import rescale_ic
...
train_window = 12
patience = 20
if not os.path.exists('./results/scale'): os.mkdir('./results/scale')
for ind in range(len(month_list) - train_window - 1):
    ...
    model = NN2net(244, drop_out=0.3)
    loss_func = rescale_ic
    ...
    if os.path.exists('./results/scale/%s' % date):
        save_data = pd.read_csv('./results/scale/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/scale/%s' % date)
