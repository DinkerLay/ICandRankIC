...
from algs.losses import ic_loss
...
train_window = 12
patience = 20
if not os.path.exists('./results/normmse'): os.mkdir('./results/normmse')
for ind in range(len(month_list) - train_window - 1):
    ...
    model = NN2Batchnorm(244, drop_out=0.3, affine=False, eta=1e-6)
    ...
    if os.path.exists('./results/normmse/%s' % date):
        save_data = pd.read_csv('./results/normmse/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/normmse/%s' % date)
