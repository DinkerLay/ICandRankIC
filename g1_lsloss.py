...
train_window = 12
patience = 20
if not os.path.exists('./results/group_lsloss'): os.mkdir('./results/group_lsloss')
for ind in range(len(month_list) - train_window - 1):
    ...
    if os.path.exists('./results/group_lsloss/%s' % date):
        save_data = pd.read_csv('./results/group_lsloss/%s' % date, index_col=0)
    ...
    save_data.to_csv('./results/group_lsloss/%s' % date)
